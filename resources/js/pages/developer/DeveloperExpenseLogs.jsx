...
export default function DeveloperExpenseLogs() {
    const [page, setPage] = useState(1);

    const { data, isLoading, isError, error, refetch } = useQuery({
        queryKey: ['developerExpenseLogs', page],
        queryFn: async () => {
            const res = await api.get('/developer/activity-logs', { params: { page, filter: 'expense' } });
            return res.data;
        },
        retry: false,
    });

    if (isLoading) return <div className="flex items-center justify-center h-40 text-emerald-600 animate-pulse font-bold">Loading expense logs...</div>;

    if (isError) {
        const message = error?.response?.data?.message || error?.message || 'Failed to load expense logs.';
        return (
    ...
    return (
        <div className="space-y-4">
            <div>
                <h2 className="text-2xl font-black text-slate-900 tracking-tight">Add Expenses Logs</h2>
                <p className="text-slate-500 text-sm font-medium mt-0.5">Monitor item photos, receipt uploads and expense submissions</p>
            </div>
            ...
            <div className="bg-white border border-slate-200/60 rounded-2xl overflow-hidden shadow-sm">
                {logs.length === 0 ? (
                    <div className="p-6 text-center">
                        <p className="text-sm text-slate-400 font-bold">No expense logs found</p>
                    </div>
                ) : (
